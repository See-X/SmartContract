...
describe(scope, () => {
    ...
    step('should deposit token successfully', async () => {
        ...
        // First, give vaultRoleC some SESToken via payout (similar to faucetUsersTUIT)
        ...
        const theSESToken = await getContractWithSignerKey('SESToken', 'nobody')
        const initialDepositorBalance = await theSESToken.balanceOf(depositorAddress)
        const contractAddress = await getContractAddress('TokenUnlockerApp')
        const initialContractBalance = await theSESToken.balanceOf(contractAddress)
        ...
        await permitERC20('SESToken', depositorAddress, 'TokenUnlockerApp', 'deployer')
        ...
        const finalDepositorBalance = await theSESToken.balanceOf(depositorAddress)
        const finalContractBalance = await theSESToken.balanceOf(contractAddress)
    ...
    step('should revert when vaultId is invalid', async () => {
        ...
        await permitERC20('SESToken', depositor, 'TokenUnlockerApp', 'deployer')
    ...
    step('should revert when depositor has insufficient balance', async () => {
        ...
        const depositorBalance = await theSESToken.balanceOf(depositor)
    ...
    step('should revert when token transfer fails due to no approval', async () => {
        ...
        // First give this user some SESToken via payout (similar to how other tests work)
        const payoutFacet = await getFacetWithSignerKey('TokenUnlockerApp', 'VaultFacet', 'vaultRoleB')
        const payoutKey = 'ecosystemDevelopment'
        const operator = await getSignerByKey(payoutKey)
        const { vaultId: payoutVaultId } = getVaultByKey(payoutKey)
        ...
        const theSESTokenWithSigner = theSESToken.connect(depositorSigner)
        
        // Explicitly revoke any existing authorization
        await theSESTokenWithSigner.approve(contractAddress, 0)
        
        // Verify no allowance exists
        const currentAllowance = await theSESToken.allowance(depositor, contractAddress)
    ...
    step('should revert when contract is paused', async () => {
        ...
        const { vaultId } = getVaultByKey('coFounders')
        const depositor = await getAccountByKey('projectReserve')
        const theSESToken = await getContractWithSignerKey('SESToken', 'projectReserve')
        const contractAddress = await getContractAddress('TokenUnlockerApp')
        
        const depositAmount = parseEther('100')
        
        // Approve tokens for transfer
        await theSESToken.approve(contractAddress, depositAmount)
    ...
    step('should handle multiple deposits to same vault correctly', async () => {
        ...
        // First deposit
        await theSESToken.approve(contractAddress, firstDeposit)
        ...
        // Second deposit
        await theSESToken.approve(contractAddress, secondDeposit)
    ...
    step('should work with different vault types', async () => {
        ...
        const theSESToken = await getContractWithSignerKey('SESToken', 'vaultRoleC')
        const contractAddress = await getContractAddress('TokenUnlockerApp')
        ...
        // Give depositor enough SESToken via payout
        const payoutFacet = await getFacetWithSignerKey('TokenUnlockerApp', 'VaultFacet', 'vaultRoleB')
        const payoutKey = 'ecosystemDevelopment'
        const operator = await getSignerByKey(payoutKey)
        const { vaultId: payoutVaultId } = getVaultByKey(payoutKey)
    ...
    step('SECURITY: should handle rapid successive deposits without state corruption', async () => {
        ...
        await theSESToken.approve(contractAddress, depositAmount.mul(numberOfRapidDeposits))

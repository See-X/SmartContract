...
contract AdminFacet_V2 is IAdminFacet, AdminBase, AccessControlBase, TTOQManagerBase, Facet {
    ...
    function profitDistribution(
        ...
        uint256 buyBackBurn = baseAmount * 40;
        uint256 stakingDividends = baseAmount * 30;
        uint256 liquidity = baseAmount * 20;
        uint256 marketMakerReward = baseAmount * 10;
        ...
        s.profitVaultMap[PayoutType.marketMakerRewardFund][profitTokenAddress] += marketMakerReward;

        // rewardFee: 90% for operatingCostsFund, 10% for riskReserveFund
        uint256 riskReserve = (rewardFeeAmount * 1) / 10;
        uint256 operatingCosts = (rewardFeeAmount * 9) / 10;
        ...
        s.profitDistributedLogMap[profitTokenAddress][dateString] = ProfitDistributedDetail({
            ...
            marketMakerReward: marketMakerReward,
